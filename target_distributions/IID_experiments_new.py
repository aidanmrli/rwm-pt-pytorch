...
if __name__ == "__main__":
    ...
    # var_value_range = np.linspace(0.0005, 0.8, 40)
    # var_value_range = np.linspace(5, 5.5, 5)
    var_value_range = [0.3852]
    # var_value_range = np.linspace(0.35, 0.4, 8)
    ...
    # target_distribution = IIDGamma(dim, shape=3, scale=2)
    target_distribution = IIDBeta(dim, alpha=3, beta=2)
    ...
    for var in var_value_range:
        ...
        a = np.mean(seed_results_acceptance)
        b = np.mean(seed_results_esjd)
        acceptance_rates.append(a)
        expected_squared_jump_distances.append(b)
        print(f"Proposal variance: {var}, Acceptance rate: {a}, ESJD: {b}")
