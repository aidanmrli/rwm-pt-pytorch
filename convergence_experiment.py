...
from target_distributions.Non_IID_converge import NONIIDBeta

if __name__ == "__main__":
    dim = 50    # dimension of the target and proposal distributions
    # run many simulations for different variance values
    # var_value_range = np.linspace(0.15, 0.5, 25)
    var_value_range = [0.27]
    num_seeds = 3
    ...
    target_distribution = NONIIDBeta(dim)
    ...
    # filename = f"images/ESJDvsVar_{target_distribution.get_name()}_ProposalUniform_RWM_dim{dim}_{num_iters}iters"
    # plt.savefig(filename)
    # plt.clf()
    # # plt.show()
    #
