...
class RandomWalkMH_GPU(MHAlgorithm):
    ...
    def _standard_step(self):
        ...
        if self.current_state is None:
            ...
            # This is the starting point, not a generated sample
        ...
        # Add the current state (after accept/reject decision) to the chain
        # This represents the sample generated by this MCMC step
    ...
    def generate_samples_standard(self, num_samples):
        ...
        if self.pre_allocated_chain is not None:
            # Return only post-burn-in samples, excluding the initial state
            # Initial state is at index 0, burn-in samples are at indices 1 to burn_in
            # Post burn-in samples are at indices (burn_in + 1) to chain_index
            return self.pre_allocated_chain[self.burn_in + 1:self.chain_index]
        else:
    ...
    def expected_squared_jump_distance_gpu(self):
        ...
        if self.pre_allocated_chain is not None and self.chain_index > 1:
            if self.chain_index > self.burn_in + 1:  # Need at least 2 post-burn-in samples
                # Exclude initial state (index 0) and burn-in samples (indices 1 to burn_in)
                chain_tensor = self.pre_allocated_chain[self.burn_in + 1:self.chain_index]
            else:
                raise ValueError(f"Insufficient post-burn-in samples: chain_index={self.chain_index}, burn_in={self.burn_in}. Need at least {self.burn_in + 3} total samples.")
        else:
